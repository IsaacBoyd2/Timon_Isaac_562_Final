...
def sampler(sig_mu,latent_space_size):  
    ...
    for k in range(latent_space_size):
        ...
        mean = sig_mu[1][k]
        sample_output = mean+sigma
        sample.append(sample_output)
    return sample,epsilons
...
def forward(sec_dat, mlp_weights,encoder_sizes,latent_space_size,decoder_sizes):
    ...
    xi.append(sig_mu)
    [plot_data,epsilons] = sampler(sig_mu,latent_space_size)
...
encoder_sizes = [num_channels,9]
latent_space_size = 2
decoder_sizes = [9,num_channels]
...
for pocs in range(epochs):
    ...
    for lk in range(len(data_holder[0])):
        ...
        [xis,epsilons] = forward(e_input,mlp_weights,encoder_sizes,latent_space_size,decoder_sizes)
        
        if pocs == epochs-1:
           graph_data.append(xis[-len(decoder_sizes)-1])
            
        #Do backwards propegation
        backprop(xis, mlp_weights,encoder_sizes,latent_space_size,decoder_sizes,eta,kl_lambda, epochs,epsilons)
...
fig = plt.figure(1)
plt.scatter(graph_data[0], graph_data[1],s = 50, c=colors, cmap='viridis', edgecolors='none', alpha=0.7)
