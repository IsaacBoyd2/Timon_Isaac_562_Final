...
def backprop(xis, mlp_weights, encoder_sizes, latent_space_size, decoder_sizes, eta, kl_lambda, epochs, epsilons,
             insta_vel, graph_data, lk, Y_val):
    ...
    for m in range(len(decoder_sizes)):
        ...
        for k in range(len(layer)):
            ...
            for z in range(len(weight_set)):
                ...
                mlp_weights[len(decoder_sizes) + len(encoder_sizes) - 1 - m][k][z] = \
                mlp_weights[len(decoder_sizes) + len(encoder_sizes) - 1 - m][k][z] + eta * deltas[-1][k] * xis[-2 - m][
                    z]
    ...
    for k in range(latent_space_size):
        # Recon + -DKL
        sig_del = epsilons[k] * deltas[-1][k] + (
                    (1 / xis[len(encoder_sizes)][0][k]) - xis[len(encoder_sizes)][0][k]) * kl_lambda
    ...
    for t in range(2):
        for k in range(latent_space_size):
            ...
            for z in range(len(weight_set)):
                mlp_weights[len(encoder_sizes) - 1][t][k][z] = mlp_weights[len(encoder_sizes) - 1][t][k][z] + eta * \
                                                               deltas[-1][count] * xis[len(encoder_sizes) - 1][z]
            count = count + 1
    ...
    for m in range(len(encoder_sizes) - 1):
        ...
        for k in range(len(layer)):
            ...
            for z in range(len(weight_set)):
                # wji = wji-eta*delta*hi
                mlp_weights[len(encoder_sizes) - 2 - m][k][z] = mlp_weights[len(encoder_sizes) - 2 - m][k][z] + eta * \
                                                                deltas[-1][k] * xis[-len(decoder_sizes) - 4 - m][z]
...
for pocs in range(epochs):
    ...
    for lk in range(len(data_holder[0])):
        ...
        cus_loss_iter = backprop(xis, mlp_weights, encoder_sizes, latent_space_size, decoder_sizes, eta, kl_lambda,
                                 epochs, epsilons, insta_vel, graph_data, lk, Y_val)
        custom_loss_tot.append(cus_loss_iter)
        # Calculate Reconstruction loss
        recon = sum((np.array(xis[-1]) - np.array(xis[0]))) ** 2
        # Calculate KL loss
        kl = (1 / 2) * (1 + math.log(xis[len(encoder_sizes)][0][0] ** 2) - xis[len(encoder_sizes)][1][0] -
                        xis[len(encoder_sizes)][0][0] ** 2)
...
pgraph_data = np.array(graph_data).T

colors = insta_vel
# Create a scatter plot with colormap
fig = plt.figure(1)
plt.scatter(pgraph_data[0], pgraph_data[1],s = 50, c=colors, cmap='Oranges', edgecolors='none', alpha=0.7)
...
fig = plt.figure(2)
plt.plot(chan1og[250:350],label='Channel Input')
plt.plot(chan1re[250:350],label='Channel Output')
#plt.ylim(0, 0.5)
plt.legend()
...
print('MSE_training')
mse_train = np.mean((np.array(chan1og)-np.array(chan1re))**2)
print(mse_train)
...
'''
output_train = []
for train in range(len(train_data_holder)):
    # print(test_data_holder[tes])

    xis = forward2(train_data_holder[train], mlp_weights, encoder_sizes)

    # print(xis)
    output_train.append(xis[-1][0])
 '''
...
'''
print('MSE_train')
mse_train = np.mean((np.array(insta_vel)-np.array(output_train))**2)
print(mse_train)
print(output_train)

fig = plt.figure(2)
plt.plot(insta_vel[250:350], label='Target velocity')
plt.plot(output_train[250:350], label='Predicted velocity')
plt.legend()

# Customize the plot with labels and title
plt.xlabel('Time')
plt.ylabel('Velocity')
plt.title('Training Regressor: VAE')
plt.show()
'''
...
plt.xlabel('Time')
plt.ylabel('Velocity')
plt.title('Testing Regression: VAE')
